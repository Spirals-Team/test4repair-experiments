...
class NodeHandler(object):
	...
	def run(self, timeoutNode=None):
		...
		while(len(self.toExecuteTask) > 0):
			...
			if self.running < self.maxNode:
				...
				print " scope %s " % (task.scope)
				print " seed NodeH %s " % (task.seed)
				filename = task.project.name.lower() + '_' + str(task.id)
				stdoutlog =  os.path.join(task.project.logPath, "seed",task.seed, task.scope, str(task.id), task.tool.name, 'stdout.log')#MM
				stderrlog =  os.path.join(task.project.logPath, "seed",task.seed, task.scope, str(task.id), task.tool.name, 'stderr.log')#MM
				resultlog =  os.path.join(task.project.logPath, "seed",task.seed, task.scope, str(task.id), task.tool.name, 'results.json')#MM
				...
				nodeCmdArgs = "%s -p %s -t %s -i %d -scope %s -seed %s" % (
	...
	def printStatus(self, totalTask, startTime):
		...
		for task in self.tasks:
			key = "%s%s%dSeed%s" % (task.project.name[0],task.scope,task.id, task.seed)#mm "%s%d" % (task.project.name[0], task.id)
			if key not in bugs:
				bugs[key] = {}
			...
			for tool in tools:
				if tool in bug:
					...
					if task in self.toExecuteTask:
						...
						resultlog =  os.path.join(task.project.logPath, "seed", task.seed,task.scope,str(task.id), task.tool.name, 'results.json')
						stderrToolPath =  os.path.join(task.project.logPath, "seed",task.seed, str(task.id),task.scope, task.tool.name, 'stderr.log')
